...
${reservedFields.map(f => `  * "${f}"`).join(`\n`)}
  ...
  if (!internalPage.component.includes(`/.cache/`) && process.env.NODE_ENV === `production` && !fileOkCache[internalPage.component]) {
    ...
    if (!fileContent.includes(`export default`) && !fileContent.includes(`module.exports`) && !fileContent.includes(`exports.default`) && !fileContent.includes(`exports["default"]`) && !fileContent.match(/export \{.* as default.*\}/s) && // this check only applies to js and ts, not mdx
