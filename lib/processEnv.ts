...
export const processEnv: ProcessEnvProps = {
  siteUrl,
  adminUrl: process.env.ADMIN_URL || siteUrl,
  platform,
  darkMode: {
    defaultMode: resolveDarkMode(
      process.env.JAMIFY_DARK_MODE_DEFAULT,
      appConfig.defaultMode
    ),
    overrideOS: resolveBool(
      process.env.JAMIFY_DARK_MODE_OVERRIDE_OS,
      appConfig.overrideOS
    ),
  },
  nextImages: {
    feature: resolveBool(
      process.env.JAMIFY_NEXT_FEATURE_IMAGES,
      appConfig.nextFeatureImages
    ),
    inline: resolveBool(
      process.env.JAMIFY_NEXT_INLINE_IMAGES,
      appConfig.nextInlineImages
    ),
    quality: resolveNumber(
      process.env.JAMIFY_NEXT_IMAGES_QUALITY,
      appConfig.imageQuality
    ),
    source: resolveBool(
      process.env.JAMIFY_NEXT_SOURCE_IMAGES,
      appConfig.sourceImages
    ),
  },
  rssFeed: resolveBool(process.env.JAMIFY_RSS_FEED, appConfig.rssFeed),
  memberSubscriptions: resolveBool(
    process.env.JAMIFY_MEMBER_SUBSCRIPTIONS,
    appConfig.memberSubscriptions
  ),
  commenting: {
    system:
      (process.env.JAMIFY_COMMENTING_SYSTEM as appConfig.CommentingSystem) ||
      appConfig.commenting,
    commentoUrl: process.env.JAMIFY_COMMENTO_URL || appConfig.commentoUrl,
    disqusShortname:
      process.env.JAMIFY_DISQUS_SHORTNAME || appConfig.disqusShortname,
  },
  prism: {
    enable: resolveBool(process.env.JAMIFY_PRISM, appConfig.prism),
    ignoreMissing: resolveBool(
      process.env.JAMIFY_PRISM_IGNORE_MISSING,
      appConfig.prismIgnoreMissing
    ),
  },
  contactPage: resolveBool(
    process.env.JAMIFY_CONTACT_PAGE,
    appConfig.contactPage
  ),
  toc: {
    enable: resolveBool(process.env.JAMIFY_TOC, appConfig.toc),
    maxDepth: resolveNumber(
      process.env.JAMIFY_TOC_MAX_DEPTH,
      appConfig.maxDepth
    ),
  },
  customNavigation: reolveJSON(
    process.env.JAMIFY_CUSTOM_NAVIGATION,
    appConfig.customNavigation
  ),
  isr: {
    enable: resolveBool(process.env.JAMIFY_NEXT_ISR, appConfig.isr),
    maxNumberOfPosts: resolveNumber(
      process.env.JAMIFY_NEXT_ISR_MAX_NUMBER_POSTS,
      appConfig.maxNumberOfPosts
    ),
    maxNumberOfPages: resolveNumber(
      process.env.JAMIFY_NEXT_ISR_MAX_NUMBER_PAGES,
