...
var _removeStaleJobs = require("./remove-stale-jobs");
...
module.exports = async args => {
  ...
  if (config && config.__experimentalThemes) {
    reporter.warn(`The gatsby-config key "__experimentalThemes" has been deprecated. Please use the "plugins" key instead.`);
  ...
  if (config && config.polyfill) {
    reporter.warn(`Support for custom Promise polyfills has been removed in Gatsby v2. We only support Babel 7's new automatic polyfilling behavior.`);
  }

  store.dispatch(_actions.internalActions.setSiteConfig(config));
  activity.end(); // run stale jobs

  store.dispatch((0, _removeStaleJobs.removeStaleJobs)(store.getState()));
  ...
  if (!process.env.GATSBY_EXPERIMENTAL_PAGE_BUILD_ON_DATA_CHANGES && process.env.NODE_ENV === `production`) {
    activity = reporter.activityTimer(`delete html and css files from previous builds`, {
      parentSpan: bootstrapSpan
    });
    activity.start();
  ...
  reporter.verbose(`Now have ${store.getState().nodes.size} nodes with ${store.getState().nodesByType.size} types: [${[...store.getState().nodesByType.entries()].map(([type, nodes]) => type + `:` + nodes.size).join(`, `)}]`);
  ...
  const graphqlRunner = (0, _createGraphqlRunner.createGraphQLRunner)(store, reporter); // Collect pages.
  ...
  reporter.verbose(`Now have ${store.getState().nodes.size} nodes with ${store.getState().nodesByType.size} types, and ${(_store$getState$nodes = store.getState().nodesByType) === null || _store$getState$nodes === void 0 ? void 0 : _store$getState$nodes.get(`SitePage`).size} SitePage nodes`);
